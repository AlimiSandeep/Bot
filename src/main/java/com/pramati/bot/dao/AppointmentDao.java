...
@Component
public class AppointmentDao implements IAppointmentService {
	...
	public String createAppointment(int doc_id, String slot_time, String appointment_date, int pid) {
		...
		String check_query = "select count(b.status) from appointments b,slots s where b.slot_id=s.slot_id and s.slot_time=? and b.doc_id=? and b.appointment_date=? and b.pid=?";

		SqlRowSet rowSet = jdbcTemplate.queryForRowSet(check_query, slot_time, doc_id, appointment_date, pid);
		rowSet.next();
